...
class SSD(HybridBlock):
    ...
    def hybrid_forward(self, F, x):
        ...
        transpose_axes =  (0, 2, 3, 1) if self.layout == 'NCHW' else (0, 1, 2, 3)
        cls_preds = [F.flatten(F.transpose(cp(feat), transpose_axes))
                     for feat, cp in zip(features, self.class_predictors)]
        box_preds = [F.flatten(F.transpose(bp(feat), transpose_axes))
